...
import { CONTENT_NORMALIZATION_ALGORITHM, measureReviewedBoundaryContent } from './reviewed_boundary_content_normalization.mjs'
...
export async function deriveContentIntegrity() {
  ...
  const records = plan.application_records.map(r => {
    ...
    const contentMeasurements = measureReviewedBoundaryContent(null, null)
    return {
      decision_id:r.decision_id, ...identity, action_classification:r.final_outcome, status_only_eligible:true, source_key:null, current_approval_status:r.expected_current_approval_status,
      ...
      word_count:null,...contentMeasurements,updated_at:null,
  ...
  const projection={schema_version:'pr0056-reviewed-boundary-content-integrity-projection-contract-v1',algorithm:'sha256-canonical-json-v1',serialization:'recursively sort JSON object keys lexicographically, preserve array order, JSON.stringify without whitespace, hash UTF-8 bytes',content_normalization:{algorithm:CONTENT_NORMALIZATION_ALGORITHM,implementation:'scripts/content_pipeline/reviewed_boundary_content_normalization.mjs',line_endings:'CRLF and CR become LF',unicode:'NFC',leading_trailing_whitespace:'trimmed per line; outer blank lines removed',repeated_internal_whitespace:'horizontal tab/vertical tab/form feed/space runs become one space',blank_lines:'three or more consecutive LF become one blank line (two LF)',non_breaking_spaces:'U+00A0 becomes ASCII space',final_newline:'exactly one LF',encoding:'UTF-8'},identity_fields:['run_id','book_id','segment_key'],boundary_fields:['source_key','segment_order','segment_index','segment_count','boundary_version','start_locator','end_locator','display_title'],content_fields:['recomputed_normalized_content_sha256','word_count','normalized_content_length','content_byte_length'],stored_digest_comparison_fields:['stored_normalized_content_sha256','stored_digest_matches_recomputed'],status_fields:['previous_approval_status','intended_resulting_approval_status'],full_pre_application_fields:['identity_sha256','boundary_sha256','content_sha256','previous_approval_status'],mutable_timestamps_excluded:['created_at','updated_at'],field_order_contract:'projection field arrays above are normative; serialized object keys are canonicalized lexicographically'}
  ...
  const classes=['MATCH','STATUS_ALREADY_APPLIED','STATUS_UNEXPECTED','IDENTITY_MISSING','BOUNDARY_DRIFT','CONTENT_DRIFT','STORED_CONTENT_DIGEST_MISMATCH','METADATA_DRIFT','SOURCE_HASH_DRIFT','DUPLICATE_TARGET','UNAUTHORIZED_TARGET','INSUFFICIENT_BASELINE']
  const drift={schema_version:'pr0056-reviewed-boundary-content-integrity-drift-contract-v1',precedence:['SOURCE_HASH_DRIFT','DUPLICATE_TARGET','UNAUTHORIZED_TARGET','IDENTITY_MISSING','INSUFFICIENT_BASELINE','STORED_CONTENT_DIGEST_MISMATCH','BOUNDARY_DRIFT','CONTENT_DRIFT','METADATA_DRIFT','STATUS_UNEXPECTED','STATUS_ALREADY_APPLIED','MATCH'],classifications:Object.fromEntries(classes.map(c=>[c,{permits_application:c==='MATCH',permits_idempotent_no_op:c==='STATUS_ALREADY_APPLIED',requires_manual_review:!['MATCH','STATUS_ALREADY_APPLIED'].includes(c),blocks_execution:!['MATCH','STATUS_ALREADY_APPLIED'].includes(c),permits_rollback:c==='STATUS_ALREADY_APPLIED',blocks_rollback:c!=='STATUS_ALREADY_APPLIED'}])),rule:'BOUNDARY_DRIFT, CONTENT_DRIFT, and STORED_CONTENT_DIGEST_MISMATCH always block status-only application and rollback'}
  ...
  const missing={schema_version:'pr0056-reviewed-boundary-content-integrity-missing-authority-v1',missing_target_count:74,missing_fields:['source_key','segment_index','segment_count','boundary_version','start_locator','end_locator','display_title','word_count','stored_normalized_content_sha256','recomputed_normalized_content_sha256','stored_digest_matches_recomputed','content_byte_length','normalized_content_length','updated_at'],reason:'No authoritative repository artifact contains the actual 74 reading_segments rows or their content; static package records contain identity, segment_order, and status only.',required_future_evidence:`A local/runtime database collector must read actual content, recompute its digest with ${CONTENT_NORMALIZATION_ALGORITHM}, compare it to stored normalized_content_sha256, classify mismatches as STORED_CONTENT_DIGEST_MISMATCH, write output under tmp/, and match unchanged authoritative source hashes. Production and remote connections are prohibited.`}
