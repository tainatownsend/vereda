...
export const paths = {
  policy: 'content/migration/reading-segment-source-review-final-unresolved-adjudication-policy.json', plan: 'content/migration/reading-segment-source-review-final-unresolved-adjudication-plan.json', decisions: 'content/migration/reading-segment-source-review-final-unresolved-adjudication-decisions.json', evidence: 'content/migration/reading-segment-source-review-final-unresolved-adjudication-evidence.json', reasons: 'content/migration/reading-segment-source-review-final-unresolved-reasons.json', impact: 'content/migration/reading-segment-source-review-final-unresolved-contract-impact.json', summary: 'content/migration/reports/reading-segment-source-review-final-unresolved-adjudication-summary.md', docs: 'docs/content-pipeline/source-review-final-unresolved-adjudication.md'
}
...
export const immutableHashPaths = {
...
export const deriveCandidateEvidence = async (records, sourcePaths = candidateSources) => {
  ...
  for (const path of sourcePaths) {
    ...
    for (const item of arr(json)) {
      ...
      const scores = candidates.map(scoreOf).filter(Number.isFinite)
      if (scores.length) {
        const max = Math.max(...scores)
        evidence.strongest_score = evidence.strongest_score == null ? max : Math.max(evidence.strongest_score, max)
        evidence.page_gap ??= candidates.find(c => scoreOf(c) === max)?.page_gap ?? item.selected_pair?.page_gap ?? null
      }
    }
  }
  for (const evidence of byId.values()) {
    ...
    if (evidence.matching_candidate_count > 0) {
      ...
      for (const s of evidence.source_artifact_summaries) {
        const json = await readJson(s.path); const item = arr(json).find(x => (x.review_packet_item_id ?? x.discovery_item_id ?? x.adjudication_id ?? x.analysis_id ?? null) === s.matching_item_id)
...
export const buildArtifacts = async () => {
  ...
  const decisions = { schema_version:'pr0052-final-unresolved-adjudication-decisions-v2', public_decision_count:144, original_unresolved_count:11, final_outcome_distribution:{...readiness.outcomeDistribution}, resolved_count:0, still_unresolved_count:11, ...confidence_counts, decisions:records }
  const reasons = { schema_version:'pr0052-final-unresolved-reasons-v2', unresolved_count:11, records:records.map(r=>({ decision_id:r.decision_id, book_id:r.book_id, book_slug:r.book_slug, packet_id:r.packet_id, current_segment_key:r.current_segment_key, final_outcome:r.final_outcome, adjudication_confidence:r.adjudication_confidence, unresolved_reasons:r.unresolved_reasons, rationale:r.outcome_specific_rationale })) }
  const impact = { schema_version:'pr0052-final-unresolved-contract-impact-v2', assessed_decision_count:11, application_ready_under_current_contracts:0, future_superseding_contract_required_count:0, records:records.map(r=>({ decision_id:r.decision_id, final_outcome:r.final_outcome, downstream_contract_lane:r.downstream_contract_lane, application_ready:r.application_ready, existing_contract_coverage:r.existing_contract_coverage, future_superseding_contract_required:r.future_superseding_contract_required })) }
  ...
  const mapping = { ...immutableHashPaths, pr0052_policy:paths.policy, pr0052_plan:paths.plan, pr0052_decisions:paths.decisions, pr0052_reasons:paths.reasons, pr0052_impact:paths.impact }; for (const p of [...readinessPaths.decisionInputs, ...candidateSources]) mapping[p.split('/').pop().replace('.json','')] = p
