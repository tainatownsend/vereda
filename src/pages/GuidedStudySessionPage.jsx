    ...
    <main className="northstar-page pb-28">
      <div className="mx-auto w-full max-w-[760px] px-5 pb-12 pt-7 sm:px-8 sm:pt-10">
        ...
        <header className="mt-4">
          <p className="text-sm font-semibold uppercase tracking-[0.12em] text-sage-700 dark:text-sage-300">Encontro {sessionIndex + 1} de {path.sessions.length} · cerca de {session.minutes} min</p>
        ...
        <div className="mt-7 min-h-[25rem]">
          {activeStep === 0 && (
            <StepPanel number="1" label="Orientação" title="Antes de começar, saiba o que observar">
              <EditorialCard className="mt-4 p-5 sm:p-6">
                <p className="text-sm font-semibold uppercase tracking-[0.08em] text-sage-700 dark:text-sage-300">Objetivo deste encontro</p>
                ...
                <div className="mt-5 border-t border-line pt-4 dark:border-night-line">
                  <p className="text-sm font-semibold uppercase tracking-[0.08em] text-sage-700 dark:text-sage-300">Durante a leitura, observe também</p>
          ...
          {activeStep === 2 && sourceAvailable && (
            <StepPanel number="3" label="Assimile" title="O que ficou da leitura?">
              <EditorialCard className="mt-4 p-5 sm:p-6">
                <p className="text-sm font-semibold uppercase tracking-[0.08em] text-sage-700 dark:text-sage-300">Em suas palavras</p>
              ...
              <EditorialCard className="mt-3 p-5 sm:p-6">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-sm font-semibold uppercase tracking-[0.08em] text-muted dark:text-night-muted">Uma conexão para guardar</p>
          ...
          {activeStep === 3 && sourceAvailable && (
            <StepPanel number="4" label="Reflexão" title="Agora é a sua vez de conversar com a ideia">
              <EditorialCard className="mt-4 p-5 sm:p-6">
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between gap-3">
                      <p className="text-sm font-semibold uppercase tracking-[0.08em] text-sage-700 dark:text-sage-300">Pergunta {reflectionPromptIndex + 1} de {reflectionPrompts.length}</p>
                      <span className="text-sm text-muted dark:text-night-muted">Sem resposta certa</span>
                    </div>
                ...
                <p className="mt-2 text-sm leading-relaxed text-muted dark:text-night-muted">Você não precisa responder às três perguntas. Elas estão aqui apenas para ajudar a ideia a assentar.</p>
          ...
          {activeStep === 4 && sourceAvailable && (
            <StepPanel number="5" label="Integração" title="Vamos assentar o ensinamento">
              ...
              <div className="mt-4 flex flex-wrap gap-2" aria-label="Ideias deste encontro">{session.concepts.map((concept) => <span key={concept} className="rounded-full bg-sage-100 px-3 py-1.5 text-sm font-medium text-sage-800 dark:bg-sage-950 dark:text-sage-300">{concept}</span>)}</div>
            </StepPanel>
          )}
...
function StepPanel({ number, label, title, children }) {
  return (
    <section aria-labelledby={`guided-step-${number}`}>
      <p className="text-sm font-semibold uppercase tracking-[0.12em] text-sage-700 dark:text-sage-300">{number} · {label}</p>
